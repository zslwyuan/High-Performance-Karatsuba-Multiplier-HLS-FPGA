...
#ifndef _karastuba_mul_resbgk_H_
#define _karastuba_mul_resbgk_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "karastuba_mul_resbgk_memcore.h"

SC_MODULE(karastuba_mul_resbgk) {
...
protected:
    ...
    karastuba_mul_resbgk_memcore* karastuba_mul_resbgk_memcore_U;
...
public:
    ...
    SC_CTOR(karastuba_mul_resbgk) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        karastuba_mul_resbgk_memcore_U = new karastuba_mul_resbgk_memcore("karastuba_mul_resbgk_memcore_U");
        karastuba_mul_resbgk_memcore_U->clk      ( clk );
        karastuba_mul_resbgk_memcore_U->reset    ( reset );
        karastuba_mul_resbgk_memcore_U->ce0      ( i_ce0 );
        karastuba_mul_resbgk_memcore_U->we0      ( i_we0 );
        karastuba_mul_resbgk_memcore_U->address0 ( memcore_iaddr );
        karastuba_mul_resbgk_memcore_U->d0       ( i_d0 );
        karastuba_mul_resbgk_memcore_U->q0       ( i_q0 );

        karastuba_mul_resbgk_memcore_U->ce1      ( t_ce0 );
        karastuba_mul_resbgk_memcore_U->we1      ( t_we0 );
        karastuba_mul_resbgk_memcore_U->address1 ( memcore_taddr );
        karastuba_mul_resbgk_memcore_U->d1       ( t_d0 );
        karastuba_mul_resbgk_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "karastuba_mul_resbgk.trace");
            // ports
