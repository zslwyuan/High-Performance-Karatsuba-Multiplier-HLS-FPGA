...
int main()
{
    ...
    for (int p=0;p<100000;p++)
    {
        ...
        gmp_randseed_ui(grt, p);
        mpz_urandomb(a, grt, 4096);
        mpz_urandomb(b, grt, 4096);
        ...
        mpz_mul(a, a, b);	
        little_end_to_array(a,my_C_check.digits.data);
        ...
        for (int i=my_C.N_ele-1;i>=0;i--)
        {
            printf("%08x",my_C.digits.data[i]);
        }
        printf("\n");
        for (int i=my_C.N_ele-1;i>=0;i--)
        {
            assert(my_C.digits.data[i]==my_C_check.digits.data[i]);
        }
        //printf("\n");
    }
